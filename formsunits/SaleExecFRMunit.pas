...
begin
  // TTC
  for i := 0 to TAdvTabSheet(MainForm.PageControlSalesCaddy.ActivePage)
    .ControlCount - 1 do
  begin
    if TAdvTabSheet(MainForm.PageControlSalesCaddy.ActivePage).Controls[i].Tag = 97
    then
    begin
      LabelTotalIndex := i;
    end;
  ...
  for nowrow := 1 to TAdvStringGrid
    (TAdvTabSheet(MainForm.PageControlSalesCaddy.ActivePage).Controls[GridIndex]
    ).Rowcount - 2 do
  begin
    model := (TAdvStringGrid(TAdvTabSheet(MainForm.PageControlSalesCaddy.
      ActivePage).Controls[GridIndex]).Cells[0, nowrow]);
    if RemoteDB.Products.Locate('products_model', model, [locaseinsensitive])
    then
    begin
      if RemoteDB.Products.FieldByName('products_street_block').AsString = 'True'
      then
      begin
        if RemoteDB.Products.FieldByName('products_date_available').AsDateTime > now
        then
          CanPrint := False;
      end;
  ...
  LabelTotal.Caption :=
    TLabel(TAdvTabSheet(MainForm.PageControlSalesCaddy.ActivePage)
    .Controls[LabelTotalIndex]).Caption;
  ...
  begin
    ...
    aCustomerDisplay.LabelCustomerBought.Caption :=
      'Produits achetés : ' + #13#10;

    for nowrow := 1 to TAdvStringGrid
      (TAdvTabSheet(MainForm.PageControlSalesCaddy.ActivePage)
      .Controls[GridIndex]).Rowcount - 2 do
    begin
      aCustomerDisplay.LabelCustomerBought.Caption :=
        aCustomerDisplay.LabelCustomerBought.Caption + #13#10 +
        (TAdvStringGrid(TAdvTabSheet(MainForm.PageControlSalesCaddy.ActivePage)
        .Controls[GridIndex]).Cells[2, nowrow]) + ' X ' +
        (TAdvStringGrid(TAdvTabSheet(MainForm.PageControlSalesCaddy.ActivePage)
        .Controls[GridIndex]).Cells[1, nowrow]) + ' à ' +
        (TAdvStringGrid(TAdvTabSheet(MainForm.PageControlSalesCaddy.ActivePage)
        .Controls[GridIndex]).Cells[4, nowrow]) + ' € ';
    end;
...
begin
  Labelpaid.Caption := FloatToStrf(NumberEditCash.Value + NumberEditBCT.Value +
    NumberEditBank.Value + NumberEditVisa.Value + NumberEditVoucher.Value,
    fffixed, 7, 2);
  Labelsolde.Caption := FloatToStrf(strtofloat(LabelTotal.Caption) -
    strtofloat(Labelpaid.Caption), fffixed, 7, 2);

  if POSBANK then
  begin
    if strtofloat(Labelsolde.Caption) < strtofloat(LabelTotal.Caption) then
    begin
      aCustomerDisplay.LabelTotalText.Caption := 'Solde : ' +
        Labelsolde.Caption;
    end
    else
    begin
      aCustomerDisplay.LabelTotalText.Caption := 'Total : ' +
        LabelTotal.Caption;
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      messagedlg('Impossible de payer plus que le montant', mtwarning,
        [mbok], 0);
    end;
  end;
  ...
  begin
    ...
    begin
      a := (MinValue([tot, (strtofloat(RemoteDB.CustomersCustomers_credit.Text)
        * 1), (tot - visa - bank - bct)]));
      b := NumberEditVoucher.Value;
      if b > a then
      begin
        NumberEditVoucher.Value :=
          MinValue([tot, strtofloat(RemoteDB.CustomersCustomers_credit.Text) *
          1, (tot - visa - bank - bct)]);
        messagedlg('Impossible de payer plus que le montant', mtwarning,
          [mbok], 0);
      end;
...
begin
  ...
  begin
    ...
    begin
      ...
      NumberEditCash.Value :=
        (strtofloat(Labelsolde.Caption) + NumberEditCash.Value);
    end;
    if Sender = SpeedButtonBCT then
    begin
      NumberEditBCT.SetFocus;
      NumberEditBCT.Value :=
        (strtofloat(Labelsolde.Caption) + NumberEditBCT.Value);
    end;
    if Sender = SpeedButtonBank then
    begin
      NumberEditBank.SetFocus;
      NumberEditBank.Value :=
        (strtofloat(Labelsolde.Caption) + NumberEditBank.Value);
    end;
    if Sender = SpeedButtonVisa then
    begin
      NumberEditVisa.SetFocus;
      NumberEditVisa.Value :=
        (strtofloat(Labelsolde.Caption) + NumberEditVisa.Value);
    end;
    if Sender = SpeedButtonVoucher then
    begin
      NumberEditVoucher.SetFocus;
      if strtofloat(RemoteDB.CustomersCustomers_credit.Text) -
        NumberEditVoucher.Value >= strtofloat(Labelsolde.Caption) then
        NumberEditVoucher.Value :=
          (strtofloat(Labelsolde.Caption) + NumberEditVoucher.Value)
      else
        NumberEditVoucher.Value :=
          (strtofloat(RemoteDB.CustomersCustomers_credit.Text) * 1);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ShowMessage
          ('Produits bloqués en street date et produits ESD sur le même ticket veuillez faire des ventes distinctes');
      end
      else
      begin
        ShowMessage
          ('Produits bloqués en street date impression ticket impossible');
        SalesProcess(False);
      end;
    end;
  end;
  ...
  try
    ...
    begin
      with TAdvStringGrid
        ...
        begin
          model := (TAdvStringGrid
            (TAdvTabSheet(MainForm.PageControlSalesCaddy.ActivePage)
            .Controls[GridIndex]).Cells[0, nowrow]);
          if RemoteDB.Products.Locate('products_model', model,
            [locaseinsensitive]) then
            if RemoteDB.Products.FieldByName('products_esd').AsInteger = 1 then
              if ((Cells[9, nowrow]).Length < 32) and (Cells[2, nowrow] = '1')
              then
              begin

                EsdAuth := ESDGate.Booktoken(Cells[0, nowrow],
                  RemoteDB.netshop_sales.FieldByName('sales_location').AsString
                  + '-' + RemoteDB.netshop_sales.FieldByName('sales_id')
                  .AsString, RemoteDB.netshop_sales.FieldByName('sales_id')
                  .AsString, False);
                if EsdAuth.status <> 1 then
                begin
                  if EsdAuth.status = -1 then
                  begin
                    ShowMessage('Plus de codes ' + Cells[1, nowrow] +
                      ' en stock. La valeur du caddy va être adaptée ');
                    Cells[2, nowrow] := '0';
                  end
                  else
                  begin
    ...
    begin
      ShowMessage
        ('Login ESD impossible les produits esd sont retirés du panier');
      with TAdvStringGrid
        (TAdvTabSheet(MainForm.PageControlSalesCaddy.ActivePage)
        .Controls[GridIndex]) do
        for nowrow := 1 to Rowcount - 2 do
    ...
    ShowMessage
      ('Erreur de connection au serveur ESD, les produits esd sont retirés du panier');
    with TAdvStringGrid(TAdvTabSheet(MainForm.PageControlSalesCaddy.ActivePage)
      .Controls[GridIndex]) do
      for nowrow := 1 to Rowcount - 2 do
      begin
        model := (TAdvStringGrid
...
var
  nowrow, Width, AvailableWidth, AvailableHeight, xpos, ypos, Left,
    Middle: integer;
  i, Num, GridIndex, SalesSavepoint, ItemsSavePoint, CustomersSavePoint,
    StockSavePoint, Check: integer;
  aRect: Trect;
...
begin
  ...
  if DBEditCustomerNbr.Text = IntToStr(ConnectedShop) then
  begin
    if self.NumberEditBank.Value > 0 then
    begin
  ...
  // check vente banque sur compte client magasin
  if NumberEditCash.Value > 3000 then
  begin
  ...
  if (RemoteDB.netshop_sales.FieldByName('sales_date_time').AsFloat > now) or
    (RemoteDB.netshop_sales.FieldByName('sales_date_time').AsFloat < now - 2)
  then
  begin
    if (RemoteDB.netshop_sales.FieldByName('sales_date_time').Value = null) then
    begin
      if messagedlg('Aucune dernière vente trouvée ! ' +
        '. Etes vous sur de vouloir enregister une vente au ' + DateToStr(now),
    ...
    begin
      ...
      if messagedlg('Dernière vente le ' +
        DateToStr(RemoteDB.netshop_sales.FieldByName('sales_date_time').Value) +
        '. Etes vous sur de vouloir enregister une vente au ' + DateToStr(now),
        mtwarning, [mbyes, mbno], 0) = mryes then
      begin
  ...
  begin
    with TAdvStringGrid(TAdvTabSheet(MainForm.PageControlSalesCaddy.ActivePage)
      ...
      begin
        ...
        begin
          if not RemoteDB.SetItemSoldAsRefunded(Trim(Cells[10, nowrow]),
            -StrToInt(Cells[2, nowrow]), False) then
          begin
            ShowMessage('Impossible d''annuler ' + Cells[1, nowrow] +
              ' operation intérompue.');
            cancontinue := False;
          end;
        end;
      end;
  ...
  begin
    ...
    try
      begin
        ...
        try
          begin
            ...
            RemoteDB.netshop_sales.FieldByName('sales_paid_cash').AsFloat :=
              NumberEditCash.Value + strtofloat(Labelsolde.Caption);
            ...
            RemoteDB.netshop_sales.FieldByName('sales_paid_voucher').AsFloat :=
              NumberEditVoucher.Value;
            RemoteDB.netshop_sales.FieldByName('sales_customer_id').AsInteger :=
              RemoteDB.Customers.FieldByName('customers_nbr').AsInteger;
            ...
            with TAdvStringGrid
              ...
              begin
                ...
                RemoteDB.netshop_items_sold.FieldByName('items_sold_model').Text
                  := Cells[0, nowrow];
                RemoteDB.netshop_items_sold.FieldByName('items_sold_name').Text
                  := Cells[1, nowrow];
                RemoteDB.netshop_items_sold.FieldByName('items_sold_quantity')
                  .Text := Cells[2, nowrow];
                RemoteDB.netshop_items_sold.FieldByName
                  ('items_sold_price_catalog').Text := Cells[3, nowrow];
                RemoteDB.netshop_items_sold.FieldByName
                  ('items_sold_price_Stock').Text := Cells[4, nowrow];
                RemoteDB.netshop_items_sold.FieldByName
                  ('items_sold_price_gross').Text := Cells[5, nowrow];
                RemoteDB.netshop_items_sold.FieldByName('items_sold_owner_id')
                  .Text := Cells[7, nowrow];
                RemoteDB.netshop_items_sold.FieldByName
                  ('items_sold_supplier_id').Text := Cells[8, nowrow];
                RemoteDB.netshop_items_sold.FieldByName('items_sold_date_in')
                  .Text := Cells[6, nowrow];
                RemoteDB.netshop_items_sold.FieldByName('items_sold_price_ht')
                  .AsFloat := RemoteDB.GetPriceHT(Cells[0, nowrow],
                  RemoteDB.netshop_items_sold.FieldByName
                  ('items_sold_price_stock').AsFloat);
                RemoteDB.Products.Locate('products_model',
                  Cells[0, nowrow], []);

                if (strtofloat(Cells[7, nowrow]) < 99999) and
                  (strtofloat(Cells[8, nowrow]) < 99999) then
                begin // Si produit neuf et que le prix n'est pas changé
                  if RemoteDB.Products.Locate('products_model',
                    Cells[0, nowrow], [locaseinsensitive]) then
                    if (strtofloat(Cells[4, nowrow])) >=
                      ((Round(RemoteDB.GetPriceTTC
                      (RemoteDB.Productsproducts_model.Text,
                      RemoteDB.Productsproducts_price.AsFloat) * 99) / 100))
                    then
                      RemoteDB.netshop_items_sold.FieldByName
                        ('items_sold_loyalty').AsInteger :=
                        RemoteDB.Products.FieldByName('products_points')
                        .AsInteger;
                end
                else
                begin // Si porduit occasion
                  RemoteDB.netshop_items_sold.FieldByName('items_sold_loyalty')
                    .AsInteger := 0;
                end;
                ...
                begin
                  try
                    if RemoteDB.Products.Locate('products_model',
                      Cells[0, nowrow], [locaseinsensitive]) then
                    begin
                      if RemoteDB.Products.FieldByName('products_esd')
                        .AsInteger = 1 then
                      begin
                        RemoteDB.netshop_items_sold.FieldByName
                          ('items_sold_esd_guid').AsString := Cells[9, nowrow];
                        if RemoteDB.netshop_items_sold.FieldByName
                          ('items_sold_quantity').AsInteger > 0 then
                        begin
                          UniqueId := Cells[9, nowrow];
                          EsdToken := ESDGate.Requesttoken(UniqueId);
                          RemoteDB.netshop_items_sold.FieldByName
                            ('items_sold_esd').AsString := EsdToken.token;
                          if (EsdToken.status = 1) then
                          begin
                          end
                          else
                          begin
                    ...
                    begin
                      ShowMessage
                        (E.ClassName + 'Erreur de connection au serveur ESD : '
                        + E.Message);
                      raise;
                    end;
                  end;
            ...
            begin
              ...
              with TAdvStringGrid
                ...
                begin
                  SqlVelocity.AddSql
                    ('UPDATE products SET products_sold_quantity = products_sold_quantity+'
                    + Cells[2, nowrow] + ' WHERE products_model=' +
                    QuotedStr(Cells[0, nowrow]) + ';');
                  SqlVelocity.AddSql
                    ('UPDATE products SET products_sold_velocity = products_sold_velocity+'
                    + IntToStr(Round(now - OscToVCLDate(Cells[6, nowrow]))) +
                    ' WHERE products_model=' + QuotedStr(Cells[0,
                    nowrow]) + ';');
                end;
            end;
            ...
            begin
              ...
              for nowrow := 1 to Rowcount - 2 do
              begin
                if (NumberEditVisa.Value = 0)
                { and (NumberEditVoucher.Value=0) }
                then
                begin
                  if (strtofloat(Cells[7, nowrow]) < 99999) and
                    (strtofloat(Cells[8, nowrow]) < 99999) then
                  begin // Si produit neuf
                    ...
                    begin
                      ...
                      RemoteDB.CustomersCustomers_points.AsInteger :=
                        RemoteDB.CustomersCustomers_points.AsInteger +
                        Trunc(strtofloat(Cells[4, nowrow]) *
                        strtofloat(Cells[2, nowrow]) / 20);
              ...
              for nowrow := 1 to Rowcount - 2 do
              begin
                if Cells[0, nowrow] = 'LOYALTY500' then
                begin
                  RemoteDB.Customers.Edit;
                  RemoteDB.Customers.FieldByName('customers_loyalty').AsFloat :=
                    RemoteDB.Customers.FieldByName('customers_loyalty').AsFloat
                    + (LoyaltyThreshold * StrToInt(Cells[2, nowrow]));
                  RemoteDB.Customers.Post;
                end;
              end;
            end;
            ...
            RemoteDB.Customers.Edit;
            RemoteDB.CustomersCustomers_credit.AsFloat :=
              RemoteDB.CustomersCustomers_credit.AsFloat -
              (NumberEditVoucher.Value * 1);
            RemoteDB.netshop_items_sold.First;
            while not RemoteDB.netshop_items_sold.eof do
            begin
              RemoteDB.Customers.FieldByName('customers_loyalty').AsInteger :=
                RemoteDB.Customers.FieldByName('customers_loyalty').AsInteger +
                (RemoteDB.netshop_items_sold.FieldByName('items_sold_loyalty')
                .AsInteger * RemoteDB.netshop_items_sold.FieldByName
                ('items_sold_quantity').AsInteger);
              ...
              with TAdvStringGrid
                ...
                begin
                  ...
                  begin
                    ...
                    begin
                      if Length(RemoteDB.Customerscustomers_email_address.
                        AsString) > 6 then
                      begin
                        if UserType = 1 then
                        begin
                          ANotifyServer := TNotifyServer.Create('deposit_sold',
            ...
            begin
              with TAdvStringGrid
                ...
                begin
                  if Trim(Cells[10, nowrow]) <> '' then
                  begin
                    if not RemoteDB.SetItemSoldAsRefunded
                      (Trim(Cells[10, nowrow]), -StrToInt(Cells[2, nowrow]),
                      True) then
                      ShowMessage('Erreur durant l''annulation de ' +
                        Cells[1, nowrow]);
                  end;
                end;
            end;
            ...
            with TAdvStringGrid
              ...
              begin
                ...
                begin
                  RemoteDB.DeleteFromStock(Cells[0, nowrow],
                    StrToInt(Cells[7, nowrow]), StrToInt(Cells[8, nowrow]),
                    StrToInt(Cells[2, nowrow]));
                end
                else
                begin
                  RemoteDB.netshop_stock.Append;
                  RemoteDB.netshop_stock.FieldByName('product_model').Text :=
                    Cells[0, nowrow];
                  RemoteDB.netshop_stock.FieldByName('product_name').Text :=
                    Cells[1, nowrow];
                  RemoteDB.netshop_stock.FieldByName('product_owner_id').Text :=
                    Cells[7, nowrow];
                  RemoteDB.netshop_stock.FieldByName('product_location')
                    .AsInteger := ConnectedShop;
                  RemoteDB.netshop_stock.FieldByName('product_quantity')
                    .AsInteger := -StrToInt(Cells[2, nowrow]);
                  RemoteDB.netshop_stock.FieldByName('product_supplier_id').Text
                    := Cells[8, nowrow];
                  RemoteDB.netshop_stock.FieldByName('product_date_in').Text :=
                    Formatdatetime('yyyymmdd', now);
                  RemoteDB.netshop_stock.FieldByName('product_price_gross').Text
                    := Cells[5, nowrow];
                  RemoteDB.netshop_stock.FieldByName('product_price_stock').Text
                    := Cells[4, nowrow];
      ...
      { Impression Ticket }
      RemoteDB.Customers.FindKey
        ([RemoteDB.netshop_sales.FieldByName('sales_customer_id').AsInteger]);
      dxStatusBar1.Panels[1].Text := 'Impression ticket';
      if ((MainForm.Parameter['PrintersTicketEnabled'] = 'TRUE') and (Ticket))
      then
      begin
...
begin
  ...
  begin
    ...
    begin
      messagedlg('Remboursement impossible ' + (Labelsolde.Caption) + ' < ' +
        FloatToStr(Value) + ' - ' + FloatToStr(NumberEditCash.Value), mtwarning,
        [mbok], 0);
      Result := False;
    end;
  end;
