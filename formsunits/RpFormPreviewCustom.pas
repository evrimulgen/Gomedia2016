...
uses
  ...
  SysUtils, Classes, RpRenderPrinter, RpRenderPreview,
  RpDefine, RpSystem, RpFormSetup, dxBar, cxClasses, System.Actions;

type
  TDavPreviewForm = class(TForm)
...
var
  DavPreviewForm: TDavPreviewForm;
...
procedure TDavPreviewForm.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  FormClosed := true;
end;

procedure TDavPreviewForm.NDRPreviewPageChange(Sender: TObject);
begin
  ...
  begin
    ...
    sbarMain.SimpleText := Trans('Page') + ' ' + IntToStr(CurrentPage) + ' ' + Trans('of') + ' ' + IntToStr(Pages);
  end; { with }
end;

procedure TDavPreviewForm.NDRPreviewZoomChange(Sender: TObject);
var
  S1: string[10];
begin
  Str(RvRenderPreview.ZoomFactor: 1: 1, S1);
  Zoomedit.Text := S1;
  if Assigned(RvRenderPreview.Image) then
  begin
    RvRenderPreview.RedrawPage;
  end; { if }
end;

procedure TDavPreviewForm.FormKeyDown(Sender: TObject; var Key: word; Shift: TShiftState);
var
  I1: integer;
begin
  case Key of
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            Position := I1;
            if (I1 > Position) and (RvRenderPreview.CurrentPage < RvRenderPreview.Pages) then
            begin
    ...
    vk_Prior:
      begin
        ...
        begin
          with ScrollBox1.VertScrollBar do
          begin
            if (Position = 0) and (RvRenderPreview.CurrentPage > 1) then
            begin
              RvRenderPreview.PrevPage;
              SetButtonState;
              I1 := Range - 1;
...
procedure TDavPreviewForm.FormKeyPress(Sender: TObject; var Key: Char);
var
  ValidKeys: string;
begin
  ...
  begin
    // PageEdit.SelStart := 0;
    // dada    PageEdit.SelLength := Length(PageEdit.Text);
    // ZoomEdit.SelStart := 0;
    // ZoomEdit.SelLength := Length(ZoomEdit.Text);
  end; { if }
end;

procedure TDavPreviewForm.actnFile_ExitExecute(Sender: TObject);
begin
  Close;
end;

procedure TDavPreviewForm.actnPage_GotoPageExecute(Sender: TObject);
var
  I1: integer;
begin
  I1 := StrToIntDef(Inputbox(Trans('Go to Page'), Trans('Enter page to move to:'), IntToStr(RvRenderPreview.CurrentPage)), -1);
  if (I1 > -1) and (RvRenderPreview.CurrentPage <> I1) then
  begin
...
procedure TDavPreviewForm.actnZoom_InExecute(Sender: TObject);
begin
  RvRenderPreview.ZoomIn;
  SetButtonState;
end;

procedure TDavPreviewForm.actnZoom_OutExecute(Sender: TObject);
begin
  RvRenderPreview.ZoomOut;
  SetButtonState;
end;

procedure TDavPreviewForm.actnPage_PreviousExecute(Sender: TObject);
begin
  RvRenderPreview.PrevPage;
  SetButtonState;
end;

procedure TDavPreviewForm.actnPage_NextExecute(Sender: TObject);
begin
  RvRenderPreview.NextPage;
  SetButtonState;
end;

procedure TDavPreviewForm.actnZoom_PageWidthExecute(Sender: TObject);
begin
  RvRenderPreview.ZoomFactor := RvRenderPreview.ZoomPageWidthFactor;
  SetButtonState;
end;

procedure TDavPreviewForm.actnZoom_PageExecute(Sender: TObject);
begin
  RvRenderPreview.ZoomFactor := RvRenderPreview.ZoomPageFactor;
  SetButtonState;
end;

procedure TDavPreviewForm.actnFile_PrintExecute(Sender: TObject);
var
  DoPrint: boolean;
  I1: integer;
  ForceCopies: integer;
begin
  if soAllowPrintFromPreview in ReportSystem.SystemOptions then
  begin
    RvRenderPrinter.NDRStream := RvRenderPreview.NDRStream;
    RvRenderPrinter.Pages     := RvRenderPreview.Pages;
    if ssAllowPreviewSetup in ReportSystem.SystemSetups then
    begin
      with TRpSetupForm.Create(self) do
        try
          PreviewSetup                       := true;
    ...
    begin
      ...
      for I1 := 1 to ForceCopies do
      begin
        if Assigned(RvRenderPrinter.NDRStream) then
        begin
...
procedure TDavPreviewForm.actnPage_FirstExecute(Sender: TObject);
begin
  RvRenderPreview.RenderPage(1);
  SetButtonState;
end;

procedure TDavPreviewForm.actnPage_LastExecute(Sender: TObject);
begin
  RvRenderPreview.RenderPage(RvRenderPreview.Pages);
  SetButtonState;
end;

procedure TDavPreviewForm.actnPage_FirstUpdate(Sender: TObject);
begin
  actnPage_First.Enabled := RvRenderPreview.CurrentPage > 1;
end;

procedure TDavPreviewForm.actnPage_PreviousUpdate(Sender: TObject);
begin
  actnPage_Previous.Enabled := RvRenderPreview.CurrentPage > 1;
end;

procedure TDavPreviewForm.actnPage_NextUpdate(Sender: TObject);
begin
  actnPage_Next.Enabled := RvRenderPreview.CurrentPage < RvRenderPreview.Pages;
end;

procedure TDavPreviewForm.actnPage_LastUpdate(Sender: TObject);
begin
  actnPage_Last.Enabled := RvRenderPreview.CurrentPage < RvRenderPreview.Pages;
end;

procedure TDavPreviewForm.FormCreate(Sender: TObject);
begin
  FPageNum := 1;
  ...
  with RvRenderPrinter do
  begin
  end; { with }
end;

procedure TDavPreviewForm.OpenFile(const AFileName: string);
begin
  with RvRenderPreview do
  begin
...
procedure TDavPreviewForm.actnFile_SaveExecute(Sender: TObject);
var
  ...
  begin
    ...
    try
      ...
      try
        ...
        if dlogSave.FilterIndex > 1 then
        begin // Do Render
...
procedure TDavPreviewForm.actnFile_OpenExecute(Sender: TObject);
begin
...
procedure TDavPreviewForm.actnFile_PrintUpdate(Sender: TObject);
begin
  actnFile_Print.Enabled := true; // RvRenderPreview.Rendering;
end;

procedure TDavPreviewForm.actnFile_SaveUpdate(Sender: TObject);
begin
  actnFile_Save.Enabled := true; // RvRenderPreview.Rendering;
end;

procedure TDavPreviewForm.actnZoom_InUpdate(Sender: TObject);
begin
  actnZoom_In.Enabled := true; // RvRenderPreview.Rendering;
end;

procedure TDavPreviewForm.actnZoom_OutUpdate(Sender: TObject);
begin
  actnZoom_Out.Enabled := true; // RvRenderPreview.Rendering;
end;

procedure TDavPreviewForm.actnZoom_PageWidthUpdate(Sender: TObject);
begin
  actnZoom_PageWidth.Enabled := true; // RvRenderPreview.Rendering;
end;

procedure TDavPreviewForm.actnZoom_PageUpdate(Sender: TObject);
begin
  actnZoom_Page.Enabled := true; // RvRenderPreview.Rendering;
end;

procedure TDavPreviewForm.InitFromRPSystem;
begin
...
class procedure TDavPreviewForm.PreviewFile(const AFileName: string);
begin
  with TDavPreviewForm.Create(Application) do
  begin
    OpenFile(AFileName);
    Visible := true;
  end; { with }
...
procedure TDavPreviewForm.ZoomEditKeyPress(Sender: TObject; var Key: Char);
begin
  if Pos(Key, { Trans- } '01234567890') > 0 then
  begin
    with Sender as TEdit do
    begin
      if (SelLength = 0) and (Pos('.', Text) > 0) and (Length(Text) > Pos('.', Text)) then
      begin
        Key := #0;
      end; { if }
    end; { with }
...
procedure TDavPreviewForm.ZoomEditExit(Sender: TObject);
var
  F1: double;
  ErrCode: integer;
begin
  Val(Zoomedit.Text, F1, ErrCode);
  if (ErrCode = 0) and (RvRenderPreview.ZoomFactor <> F1) then
  begin
    RvRenderPreview.ZoomFactor := F1;
  end; { if }
end;

procedure TDavPreviewForm.PageEditExit(Sender: TObject);
var
  I1: integer;
  ErrCode: integer;
begin
  Val(PageEdit.Text, I1, ErrCode);
  if (ErrCode = 0) and (RvRenderPreview.CurrentPage <> I1) then
  begin
    RvRenderPreview.RenderPage(I1);
  end; { if }
end;

procedure TDavPreviewForm.PageEditKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = '.' then
  begin
    Key := #0;
...
procedure TDavPreviewForm.SetSaveOptions(SaveDialog: TSaveDialog);
begin
...
procedure TDavPreviewForm.PreviewTimerTimer(Sender: TObject);
begin
...
procedure TDavPreviewForm.NewRenderPage(Sender: TObject);
begin
  // Called when a new render page is available

  // Update page count
  // DADA  PageLabel.Caption := Trans('of') + ' ' + IntToStr((Sender as TRvNDRWriter).JobPages);

  if PageNum = 1 { (Sender.Pages = PageNum) } then
  begin
    // Draw page content
...
procedure TDavPreviewForm.FormDestroy(Sender: TObject);
begin
  FreeAndNil(RvRenderPreview);
  FreeAndNil(RvRenderPrinter);
end;

procedure TDavPreviewForm.AfterPreviewPrintEvent;
begin
  if Assigned(FOnAfterPreviewPrint) then
  begin
    OnAfterPreviewPrint(RvRenderPrinter);
  end; { if }
end;

procedure TDavPreviewForm.SetButtonState;
begin
...
procedure TDavPreviewForm.FormShow(Sender: TObject);
begin
  SetButtonState;
  actnZoom_PageWidthExecute(self);
end;
