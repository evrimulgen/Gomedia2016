...
begin
  cxCurrencyEdit.Value := (RemoteDB.Customers.Fieldbyname('customers_credit')
    .Value / ((MarginDepositVoucher) / 100) * ((MarginDepositRefund) / 100));
end;
...
begin
  RefundValid := False;
  if cxCurrencyEdit.Value - 0.01 >
    (RemoteDB.Customers.Fieldbyname('customers_credit').Value /
    (MarginDepositVoucher / 100) * (MarginDepositRefund / 100)) then
  begin
    cxCurrencyEdit.Value := (RemoteDB.Customers.Fieldbyname('customers_credit')
      .Value / (MarginDepositVoucher / 100) * (MarginDepositRefund / 100));
  ...
  begin
    // Verification type d'opération

    if (Mainform.Parameter['MiscSIGNPADUse'] = 'TRUE') then
    begin
        ...
        try
          try
            begin
              ...
              FormSign.TextLines.Add('Client # : ' +
                RemoteDB.CustomersCustomers_nbr.Text);
              FormSign.TextLines.Add
                ('Nom : ' + RemoteDB.CustomersCustomers_firstname.Text);
              FormSign.TextLines.Add('Ancien bon : ' +
                RemoteDB.CustomersCustomers_credit.Text);
              FormSign.TextLines.Add('Nouveau bon : ' +
                floattostrf((RemoteDB.CustomersCustomers_credit.AsFloat -
                (cxCurrencyEdit.Value * (MarginDepositVoucher / 100) /
          ...
          if FormSign.ModalResult = mrOK then
            RefundValid := True;
    ...
    begin
      ...
      begin
        begin
          ...
          try
            TicketToPrint.Printer := Mainform.Parameter
              ['PrintersTicketPrinter'];
            ...
            TicketToPrint.AddFinancialLine('Remboursement :',
              cxCurrencyEdit.Value);
    ...
    begin
      ...
      RemoteDB.netshop_refunds.Fieldbyname('refunds_amount').Value :=
        -cxCurrencyEdit.Value;
      ...
      if (Mainform.Parameter['MiscSIGNPADUse'] = 'TRUE') then
        RemoteDB.StoreSignBlob('signature.jpg', 'refunds');
