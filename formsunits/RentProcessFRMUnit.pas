...
begin
  ...
  for i := 0 to TAdvTabSheet(Mainform.PageControlRentCaddy.ActivePage).ControlCount - 1 do
  begin
    if TAdvTabSheet(Mainform.PageControlRentCaddy.ActivePage).Controls[i].ClassType = TAdvStringGrid then
    begin
      GridIndex := i;
    end;
  end;

  if RemoteDB.Customers.Locate('customers_nbr', Mainform.GetRentCaddyCustomerNbr, []) then
  begin

    if RentOperation = RentOut then
    begin

      RemoteDB.netshop_rent_transaction.Filter := 'rent_transaction_returned= 0  and rent_transaction_customer_nbr = ' +
      ...
      with TAdvStringGrid(TAdvTabSheet(Mainform.PageControlRentCaddy.ActivePage).Controls[GridIndex]) do
        ...
        begin
          ...
          begin
            ...
            RemoteDB.netshop_rent_schemes.FieldByName('rent_scheme_base_price').AsString := Cells[5, NowRow];
            RemoteDB.netshop_rent_schemes.FieldByName('rent_scheme_extra_price').AsString := Cells[6, NowRow];
            RemoteDB.netshop_rent_schemes.GotoKey;
            FieldByName('rent_transaction_scheme_id').Value := RemoteDB.netshop_rent_schemes.FieldByName('rent_scheme_id').Value;
            Post;
          end;

          with RemoteDB.netshop_rent_stock do
          begin
            Filter := '(rent_stock_product_model = ' + quotedstr(RemoteDB.netshop_rent_transaction.FieldByName('rent_transaction_product_model').Value) +
            ...
            begin
              ...
              FieldByName('rent_stock_last_rent').Value := Strtodate(TAdvStringGrid(TAdvTabSheet(Mainform.PageControlRentCaddy.ActivePage).Controls[GridIndex])
                .Cells[3, NowRow]);
    ...
    begin
      ...
      with TAdvStringGrid(TAdvTabSheet(Mainform.PageControlRentCaddy.ActivePage).Controls[GridIndex]) do
        ...
        begin
          with RemoteDB.netshop_rent_transaction do
          begin
            if Locate('rent_transaction_product_model', Cells[0, NowRow], []) then
            begin
...
var
  Num, NowRow, i, j, nowrowsales, SalesPage, SalesGridIndex, RentGridIndex, OverDueDays: integer;
  Data: WideString;
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        ...
        begin
          try
            ...
            begin
              PosPrinter.PrintNormal(2, chr(27) + 'a' + chr(0) + RemoteDB.netshop_rent_transaction.FieldByName('rent_transaction_product_name').AsString +
                chr(10) + chr(13));
              PosPrinter.PrintNormal(2, chr(27) + 'a' + chr(50) + 'Du ' + DatetoStr(RemoteDB.netshop_rent_transaction.FieldByName('rent_transaction_start_date')
                .AsFloat) + ' Au ' + DatetoStr(RemoteDB.netshop_rent_transaction.FieldByName('rent_transaction_due_date').AsFloat) + chr(10) + chr(13));
            ...
            PosPrinter.PrintNormal(2, chr(27) + '!' + chr(57) + chr(27) + '|cA' + WEBURL + chr(13));
            Randomize;
      ...
      begin
        for j := 0 to TAdvTabSheet(Mainform.PageControlSalesCaddy.Pages[i]).ControlCount - 1 do
        begin
          if Mainform.PageControlSalesCaddy.Pages[i].Controls[j].Tag = 100 then
          begin
            if TEdit(Mainform.PageControlSalesCaddy.Pages[i].Controls[j]).Text = RemoteDB.Customerscustomers_nbr.AsString then
            begin
              SalesPage := i;
            end;
          end;
      ...
      for i := 0 to TAdvTabSheet(Mainform.PageControlSalesCaddy.Pages[SalesPage]).ControlCount - 1 do
      begin
      ...
      for NowRow := 1 to TAdvStringGrid(TAdvTabSheet(Mainform.PageControlRentCaddy.ActivePage).Controls[RentGridIndex]).Rowcount - 2 do
      begin
        // Add In the sales
        with TAdvStringGrid(TAdvTabSheet(Mainform.PageControlSalesCaddy.Pages[SalesPage]).Controls[SalesGridIndex]) do
        begin
          nowrowsales           := Rowcount - 1;
          Cells[0, nowrowsales] := 'LOCA';
          Cells[1, nowrowsales] := 'Location : ' + TAdvStringGrid(TAdvTabSheet(Mainform.PageControlRentCaddy.ActivePage).Controls[RentGridIndex])
            .Cells[1, NowRow];;
  ...
  begin
    ...
    for NowRow := 1 to TAdvStringGrid(TAdvTabSheet(Mainform.PageControlRentCaddy.ActivePage).Controls[RentGridIndex]).Rowcount - 2 do
    begin
      if RemoteDB.netshop_rent_transaction.Locate('rent_transaction_product_model',
        TAdvStringGrid(TAdvTabSheet(Mainform.PageControlRentCaddy.ActivePage).Controls[RentGridIndex]).Cells[0, NowRow], []) then
      begin
        if Round(RemoteDB.netshop_rent_transaction.FieldByName('rent_transaction_due_date').Value) < Round(Now) then
        begin
          OverDueDays := Round(Now) - Round(RemoteDB.netshop_rent_transaction.FieldByName('rent_transaction_due_date').Value + 1);
          ...
          OverDueAmount := OverDueDays * RemoteDB.netshop_rent_schemes.FieldByName('rent_scheme_extra_price').Value;
          // Add In the sales
          with TAdvStringGrid(TAdvTabSheet(Mainform.PageControlSalesCaddy.Pages[SalesPage]).Controls[SalesGridIndex]) do
          begin
            nowrowsales           := Rowcount - 1;
            Cells[0, nowrowsales] := 'LOCA';
            Cells[1, nowrowsales] := 'Retard : ' + TAdvStringGrid(TAdvTabSheet(Mainform.PageControlRentCaddy.ActivePage).Controls[RentGridIndex])
              .Cells[1, NowRow];;
            Cells[2, nowrowsales] := '1';
