...
type
  TFormPrepareOrderWizard = class(TForm)
    ...
    procedure JvExpressButtonAdminOrderSaveClick(Sender: TObject);
...
procedure TFormPrepareOrderWizard.JvExpressButtonAdminOrderSaveClick(
  Sender: TObject);
...
begin
 ...
 try
  ...
  aSaveDialog.FileName:='order-'+ComboboxSuppliers.SelText+'-'+datetostr(now)+'.csv';
  if ASaveDialog.Execute then begin
   StringGridAdminOrder.SaveToCSV(aSaveDialog.FileName);
  end;
 finally
  aSaveDialog.Free;
 end;
